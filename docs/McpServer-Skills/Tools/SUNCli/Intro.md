...
| **SunPump Token Launch** | Create a meme token in one command via the SunPump agent endpoint — server-side creation, **no wallet needed** (TRON mainnet only) | `sun sunpump launch --name "<name>" --symbol "<SYMBOL>" --description "..."` |

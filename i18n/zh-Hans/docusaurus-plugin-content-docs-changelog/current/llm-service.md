---
title: 'LLM Service'
description: 'LLM Service 的版本发布记录。'
---
...
- 新增 **GPT-5.6** 系列——`sol`、`terra`、`luna`，三者定价均已列出。
...
### Claude Fable 5 与 Claude Sonnet 5

<div className="changelog-tags"><span className="changelog-tag">新模型</span></div>

- 新增 **Claude Fable 5** 与 **Claude Sonnet 5**，定价完整。
...
- 已停用的模型从模型列表和定价页移除。继续调用这些模型名会直接报错，请切换到在售模型。
...
### MiniMax M3 与 Claude Opus 4.8

<div className="changelog-tags"><span className="changelog-tag">新模型</span></div>

- 新增 **MiniMax M3** 与 **Claude Opus 4.8**，同时重排模型侧边栏，同系列模型归到一起。

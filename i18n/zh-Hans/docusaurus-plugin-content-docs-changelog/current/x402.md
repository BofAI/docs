...
- **支持 Base**——`eip155:8453`（主网）支持 USDC，并提供 `base-mainnet` 别名。Base 走标准 `exact` 方案，但使用 **EIP-3009** 授权而非 Permit2。生产环境请通过 `--rpc-url` 或 `EVM_RPC_URL_8453` 自备 RPC。
...
- 发布 `eip155:8453` **Base 主网目录路由**，配套支付指引与代码示例。

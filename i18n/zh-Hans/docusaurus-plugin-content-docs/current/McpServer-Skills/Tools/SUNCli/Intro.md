...
| **SunPump 行情** | meme 代币发现：排行、详情、持有人、钱包持仓、交易历史（仅 TRON 主网） | `sun sunpump token ranking --type MARKET_CAP --size 10` |
...
| **SunPump 一键发币** | 通过 SunPump agent 端点一条命令创建 meme 代币——服务端签名创建，**无需钱包**（仅 TRON 主网） | `sun sunpump launch --name "<name>" --symbol "<SYMBOL>" --description "..."` |
| **SunPump 发射前交易** | 在 SunPump 上买卖还没在 SunSwap V2 上建交易对的 meme 代币（仅 TRON 主网） | `sun sunpump buy <address> --trx 10` |
...
:::tip 什么时候选哪个？
如果你的 AI 助手支持 MCP 协议，优先选 SUN MCP Server——集成更简洁，无需处理命令拼接和输出解析。如果你需要在 Shell 脚本、定时任务、CI/CD 流水线中使用，或者你的 AI Agent 不支持 MCP 协议，选 SUN CLI。
:::

...
| **sunpump-agent-skill** | SunPump meme 币：查行情/排行/持有人/钱包持仓，买卖 meme 币（根据代币是否已发射自动选择兑换路线，仅 TRON 主网） | 只读查询免配置；链上买卖需钱包凭证 |
...
想玩 SunPump 上的 meme 币？这个技能基于 `@bankofai/sun-cli`，帮你查行情、做研究、买卖 meme 币。最关键的是它会**自动判断交易路径**：还没在 SunSwap V2 上创建交易对的「发射前」代币走 `sun sunpump buy/sell`，已在 SunSwap V2 建好交易对的「发射后」代币走普通 `sun swap`——下单前 AI 会先用 `sunpump state` 帮你确认走哪条路。**sunpump-agent-skill 的所有功能（查询和交易）都只支持 TRON 主网，不支持测试网。**

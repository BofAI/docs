...
| **sunpump-agent-skill** | SunPump meme 币：查行情/排行/持有人/钱包持仓，在 SunPump 上买卖打新（仅 TRON 主网） | 只读查询免配置；链上买卖需钱包凭证 |
...
想玩 SunPump 上的 meme 币？这个技能基于 `@bankofai/sun-cli`，帮你查行情、做研究、在 SunPump 上买卖打新。最关键的是它会**自动判断交易路径**：还没在 SunSwap V2 上创建交易对的「发射前」代币走 `sun sunpump buy/sell`，已在 SunSwap V2 建好交易对的「发射后」代币走普通 `sun swap`——下单前 AI 会先用 `sunpump state` 帮你确认走哪条路。**sunpump-agent-skill 的所有功能（查询和打新买卖）都只支持 TRON 主网，不支持测试网。**
...
> 帮我在 SunPump 上用 10 TRX 打新买入 TXYZ...（还没发射到 SunSwap 的代币）。
...
> 想打新？ "TXYZ... 现在是什么状态？如果还能打新，帮我用 10 TRX 买入。"——AI 会先查 `state`，再报价，确认后才下单。
...
:::tip GasFree 支持（TRON）
在 TRON 上付款时，技能会优先选择 `exact_gasfree` 方案（比 `exact_permit` 更便宜）。GasFree 需要账户已激活且 GasFree 钱包里有足够余额——用 `--gasfree-info` 检查，没激活就用 `--gasfree-activate` 激活。
:::

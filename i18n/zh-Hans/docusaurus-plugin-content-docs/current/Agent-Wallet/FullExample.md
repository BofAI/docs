...
1. 已安装 Agent-wallet SDK（参见 [SDK 快速开始](./SDKQuickStart.md)）
2. 已通过 CLI 初始化本地钱包，或已配置静态模式环境变量
3. 已设置 `AGENT_WALLET_PASSWORD`（本地模式）或 `AGENT_WALLET_PRIVATE_KEY`（静态模式）
...
（Agent-wallet Python SDK 已包含，无需单独安装。）

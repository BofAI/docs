...
| :------- | --------------------: | ------------------------: | ------------------------: | -------------------: | ---: | :--- |
| **空闲时段** | `$0.66` | `$0.66` | `$0.022` | `$1.98` | - | 缓存写入：输入价的 `1x`；缓存读取：输入价的 `0.0333x` |
| **高峰时段** | `$1.32` | `$1.32` | `$0.044` | `$3.96` | - | 缓存写入：输入价的 `1x`；缓存读取：输入价的 `0.0333x` |

**Credits 结算：** B.AI 按照 `1 USD = 1,000,000 Credits` 换算，并从账户余额中扣除 Credits。

:::info 价格说明
DeepSeek V4 Pro 采用分时标准参考价。空闲时段价格为高峰时段价格的一半。北京时间（UTC+8）周一至周五（不含中国法定节假日）09:00-12:00、14:00-18:00 为高峰时段；其余时段，包括周末及中国法定节假日全天，均为空闲时段。B.AI Chat 中的 DeepSeek V4 Pro 按空闲时段价格计费。实际结算价格及最终账单以平台页面展示为准。B.AI 可能会通过充值赠送及账户权益等方式，为用户提供更低的实际使用成本。
:::

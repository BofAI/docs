...
所有的支付载荷均由买方**使用 TIP-712/EIP-712 进行签名**，且结算过程**直接在区块链上**执行。任何试图篡改交易数据的 Facilitator 都无法通过链上的签名验证。Facilitator 仅有权执行以下操作：
...
3.  使用 x402 客户端 SDK 对 TIP-712/EIP-712 支付载荷进行签名。
4.  携带包含签名的 `PAYMENT-SIGNATURE` 标头再次发起请求。

#### 代理需要钱包吗？

**需要。** 程序化钱包（通过 x402 提供的签名器类实现）允许代理对 TIP-712/EIP-712 载荷进行签名，且**无需直接暴露助记词**，从而确保资金安全。

### 开发指南

#### 如何在本地运行 x402？

1.  **克隆仓库：** 下载 [x402-demo 仓库](https://github.com/bankofai/x402-demo) 到本地。
...
1.  **TIP-712/EIP-712 签名无效**：域 (Domain) 参数配置错误或载荷 (Payload) 字段不匹配。

...
| BSC 主网 (`eip155:56`)      | USDT (BEP-20) | **主网**   |
| BSC 测试网 (`eip155:97`)    | USDT (BEP-20) | **测试网** |
...
x402 使用统一的 `exact` 支付方案。它允许客户端授权特定金额，服务端通过 Facilitator 进行结算。这适用于**按次付费**、**按量计费**以及 **AI 代理自主支付**等场景。

#### x402 支持定期扣款（订阅制）吗？

目前 x402 侧重于单次请求或单次会话的授权。虽然目前还不支持传统的“无限期”定期自动扣款，但一次授权可以用于支付方案配置的多步结算。完全的定期订阅支持已列入我们的长期路线图。

#### x402 兼容所有代币吗？

x402 兼容大多数标准的 TRC-20 和 BEP-20 代币。`exact` 方案会自动适配代币的功能，包括标准转账、EIP-2612 Permit 以及 EIP-3009 原生授权。

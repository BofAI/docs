...
- **准备支付载荷**：根据服务端的要求，构建并签署 TIP-712/EIP-712 支付载荷。
...
- **验证支付载荷**：调用 Facilitator 服务对传入的 TIP-712/EIP-712 签名载荷进行验证。
...
3.  **客户端提交支付**：根据要求生成 TIP-712/EIP-712 签名，并将签名载荷放入 `PAYMENT-SIGNATURE` 标头（Base64 编码）中重新发送请求。
...
- **客户端 (Client)**：发起资源请求，并负责提供经过 TIP-712/EIP-712 签名的支付载荷。

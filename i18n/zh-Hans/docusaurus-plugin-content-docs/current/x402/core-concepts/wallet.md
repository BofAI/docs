...
根据代币协议的不同，客户端可能需要授权 Facilitator 或代理合约来划转代币。

- **EIP-3009 (原生授权)**：无需单独的 `approve` 交易。授权信息已包含在签署的支付载荷中。
- **EIP-2612 (Permit)**：SDK 处理 permit 签名，这取代了手动的 `approve` 交易。
- **标准 ERC-20 / TRC-20**：对于传统代币，客户端必须通过标准的 `approve` 函数授权 `Permit2` 合约（x402 用于统一结算）。

x402 客户端 SDK 会自动检测所需的流程，并根据需要提示进行授权或签名。

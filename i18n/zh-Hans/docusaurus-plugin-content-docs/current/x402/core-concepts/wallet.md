...
在 x402 协议中，钱包不仅是资金的容器，更是买家（客户端）和卖家（服务端）在去中心化网络中的**核心身份标识**。
...
- **接收资金**：作为 USDT/TRC-20/BEP-20 支付的最终结算目的地。
- **配置目标**：在服务端配置中明确定义收款地址。

卖家的钱包地址会直接包含在 `HTTP 402` 响应的支付要求 (Payment Requirements) 中，确保资金流向透明。
...
## BSC 地址格式
BSC 网络使用 Hexadecimal (十六进制) 编码格式的地址，且固定以字符 0x 开头。此格式与 Ethereum（以太坊）完全兼容。

- 示例：0x55d3...97955（Hex 编码，以 0x 开头）
...
- **严禁暴露私钥**：官方 CLI 的账户与签名由 [wallet-cli](/zh-Hans/wallet-cli/quickstart/)管理；现有 SDK 集成可使用 [Agent Wallet](/zh-Hans/Agent-Wallet/Intro/) 或其他受支持的签名器。不要将私钥写入源代码、命令参数或聊天。
...
- **核心基础**：钱包是 x402 协议实现程序化、无许可支付的基础设施。
- **买家操作**：买家通过钱包生成签名，授权并支付服务费用。
- **卖家接收**：卖家利用钱包地址作为接收资金的终端。

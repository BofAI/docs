...
| 网络名称 (Network Name) | x402 ID   | 描述 (Description)   |
| :---------------------- | :------------- | :------------------- |
| **BSC Mainnet**        | `bsc:mainnet` | BSC 主网 (生产环境) |
| **BSC Chapel**         | `bsc:chapel`  | BSC Chapel 测试网   |


## 概览

x402 专为区块链生态设计，实现了原生的链上支付验证与结算功能。协议底层严格采用 **TIP-712/EIP-712** 标准，确保消息签名的安全性与防篡改能力。
...
| **BSC Chapel**         | **Testnet**   | **推荐测试网**：BSC 首选的开发与调试环境。 |
### 支持的代币
...
| **USDT** | `bsc:chapel`    | `0x337610d27c682E347C9cD60BD4b3b107C9d34dDd` |

> **扩展支持**：协议具有高度的可扩展性。通过在 `TokenRegistry` 中进行注册，您可以轻松配置并支持任意自定义的 TRC-20/BEP-20 代币。

#### 基于 TIP-712/EIP-712 的安全签名

x402 严格采用 **TIP-712/EIP-712** 标准来处理所有支付相关的结构化数据签名。
...
1.  **预授权 (Authorize)**：客户端签署 TIP-712/EIP-712 消息，授权支付**最大金额** (Max Amount)。
...
1.  **验证载荷 (Verify)**：校验 TIP-712/EIP-712 签名的加密有效性及参数完整性。
...
| 核心组件     | TRON/BSC 实现详情                              |
| :----------- | :----------------------------------------- |
| **网络环境** | `tron:mainnet`, `tron:shasta`, `tron:nile`,`bsc:mainnet`, `bsc:chapel`|
| **代币标准** | TRC-20 代币（默认内置 USDT 和 USDD 支持）,BEP-20 代币 |
| **签名机制** | TIP-712/EIP-712 结构化数据签名                     |
| **支付方案** | `exact`                                    |
...
x402 专为 TRON 区块链架构深度定制，提供了原生的 TRC-20/BEP-20 代币集成与 TIP-712/EIP-712 签名支持。

**核心要点：**

- **开发环境**：推荐优先使用 **测试网** 进行开发与调试。
- **原生资产**：**USDT** 为默认的首选结算代币，且 SDK 已预置相关合约地址配置。
- **安全机制**：**TIP-712/EIP-712** 机制确保了安全且最小化信任 (Trust-minimized) 的支付授权流程。

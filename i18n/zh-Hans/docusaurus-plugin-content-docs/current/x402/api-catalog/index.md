...
| 服务 | 它能做什么 | 计费 |
|---|---|---|
| SunPump | 一次付费完成 Agent/meme 代币发行 —— 提交发币元数据（名称、符号、描述、图片），支付结算后由网关转发你的请求 | $0.001 / 次 |
| DefiLlama | 聚合 DeFi 数据 —— 协议 TVL、手续费、代币价格（实时与历史）、池子收益/APY | $0.001 / 次 |
| DexScreener | 实时 DEX 交易对、价格与流动性、代币搜索，以及最新上新档案 | $0.001 / 次 |
| DIA | 去中心化、多源预言机价格，按符号或链 + 合约地址查询 | $0.001 / 次 |
| GoPlus | 代币与地址安全检测 —— 蜜罐/诈骗筛查、恶意地址与高风险授权识别 | $0.001 / 次 |
